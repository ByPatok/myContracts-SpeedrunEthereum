...
const Streamer: NextPage = () => {
  ...
  function recieveVoucher(clientAddress: string) {
    ...
    async function processVoucher({ data }: { data: Pick<Voucher, "signature"> & { updatedBalance: string } }) {
      ...
      if(!verified) {    
        console.error(`Voucher signature verification failed for client ${clientAddress}`);
        return;
      }
  ...
  const wisdomSizes: { [key: string]: number} = {};
  const paymentSizes: {[key: string]: number} = {};
  const threshold = 0.5;

  const provideService = (client: AddressType, wisdom: string) => {
    const wisdomSize = wisdom.length;
    wisdomSizes[client] = (wisdomSizes[wisdom.length] || 0) + wisdomSize;
    const paymentSize = paymentSizes[client] || 0;
    if (wisdomSizes[client] > paymentSize + threshold){
      return;
    }
